...
var is_side_note : bool

@warning_ignore("shadowed_variable")
func _init(start_time : float, end_time : float, idx : int, type : Type, powered : bool, is_valid : bool, is_selected : bool, is_side_note : bool) -> void:
	...
	self.is_side_note = is_side_note

func get_dictionary() -> Dictionary:
	return {"start_time": start_time, "end_time": end_time, "idx": idx, "type": type, "powered": powered, "is_valid": is_valid, 
		"is_selected": is_selected, "is_side_note": is_side_note}

static func dictionary_to_resource(dict : Dictionary) -> NoteResource:
	return NoteResource.new(dict["start_time"], dict["end_time"], dict["idx"], dict["type"], dict["powered"], dict["is_valid"], dict["is_selected"], dict["is_side_note"])
		...
		match type:
			...
			Type.HOLD:
				if not is_side_note:
					...
					note = SideNote.new(start_time, end_time, 0)
		...
		match type:
			...
			Type.HOLD:
				if not is_side_note:
					...
					note = SideNoteEditor.new(start_time, end_time)
	note.set_idx(idx)
