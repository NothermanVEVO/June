...
var _holder_size_type : NoteHolder.Holder
...
func get_width_by_holder_type() -> float:
	return NoteHolder.width if _holder_size_type == NoteHolder.Holder.NOTES else Gear.width / 2

func get_height_by_holder_type() -> float:
	return height if _holder_size_type == NoteHolder.Holder.NOTES else SideNote.side_height

func set_start_time(start_time : float) -> void:
	set_time(start_time)
	
	_start_note.size = Vector2(get_width_by_holder_type(), get_height_by_holder_type() / 2.0)
	_start_note.position = Vector2(0, _start_note.size.y)
	
	## FOR SIDE NOTES
	#if _holder_size_type == NoteHolder.Holder.SIDE_NOTES:
		#_start_note.position.y += get_height_by_holder_type()
...
func set_end_time(end_time : float) -> void:
	...
	## FOR SIDE NOTES
	#if _holder_size_type == NoteHolder.Holder.SIDE_NOTES:
		#_end_note.position.y += get_height_by_holder_type()
	...
	## FOR SIDE NOTES
	#if _holder_size_type == NoteHolder.Holder.SIDE_NOTES:
		#_middle_note.size.y -= get_height_by_holder_type()
